...
export type Types<Message, Together extends TogetherInterface<Message>> = {
  context: ChatPromptContext<Message, Together>;
  result: ChatCompletionCreateResult<Message, Together>;
  inferenceParams: InferenceParams<ChatPromptContext<Message, Together>>;
};
 ...
 *        type expectations.
 */
export const buildChatInferenceFunctionsForTogether = <
  Message,
  Together extends TogetherInterface<Message>,
  T extends Types<Message, Together>,
>(
  together: Together,
) => {
  ...
  >(
    ...
    if (jsonModeSupportedModels.includes(initialContext.body.model)) {
      ...
      choice = await inferChatRaw(context.messages ?? [], context);
    }
